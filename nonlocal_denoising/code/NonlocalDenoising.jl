...
function nlMeans(I,patchRadius,searchRadius, support, sigma)
  kernel = makeKernel(patchRadius,sigma)
  kernel = kernel[:]
    ...
    for c=patchRadius+1:size(I,2)-patchRadius
      ...
      for i=searchCorner[1]:searchCorner[1]+searchSide
        for j=searchCorner[2]:searchCorner[2]+searchSide
          ...
          searchPatch = searchPatch[:] .* kernel
